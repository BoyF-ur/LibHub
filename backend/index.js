...
app.get("/search-books", async (req, res) => {
    ...
    try {
        ...
        const totalPage = Math.ceil(countRecords / parseInt(limit));
        res.status(200).json({
            stories: books,
...
app.post("/image-upload-url", async (req, res) => {
  try {
    ...
    if (!/^https?:\/\/.+\.(jpg|jpeg|png|gif|webp|avif|jfif)$/i.test(imageUrl)) {
      return res
        .status(400)
        .json({ error: true, message: "Invalid image URL format" });
    }
...
app.post("/borrow/:id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (req.body.borrowNumber > book.remainingBook) {
      return res
        .status(400)
        .json({ error: true, message: "Not enough book to borrow" });
    } else {
...
app.delete("/delete-borrow/:id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!borrow) {
      return res
        .status(404)
        .json({ error: true, message: "Borrow request not found" });
    }
